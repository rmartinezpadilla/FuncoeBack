...
from schemas.pensum import Pensum as pensum_schema
from config.db import get_db,Session
from models.pensum import Pensum as pensum_model
import uuid
from datetime import datetime

router =  APIRouter(prefix='/pensum', tags=['Pensum'], responses={404 : {'message' : 'Not found'}})
...
def create_pensum(pensum_obj:pensum_schema):
    ...
    #¡inicio try!
        ...
        for db in session:
            ...
            pensum_obj.uuid_advisor = uuid.uuid4()
...
@router.get("/{uuid_pensum}", response_model = pensum_schema)
...
# @router.get("/{identification_card}", response_model = pensum_schema)
...
#                 #encontrar la primera coincidencia, esto es posible porque el id es un 
#                 #identificador unico
#                 r=db.query(pensum_model).where(pensum_model.identification_card == id_card).first()
#                 #r=db.select(pensum_model).where(pensum_model.identification_card == id_card)
#                 return r
#         #¡fin try!
#     except Exception as e:#instrucción que nos ayuda a atrapar la excepción que ocurre cuando alguna instrucción dentro de try falla
#             #se debe controlar siempre que nos conectamos a una base de datos con un try - except
