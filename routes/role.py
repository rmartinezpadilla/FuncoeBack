...
router =  APIRouter(prefix='/roles', tags=['Roles'], responses={404 : {'message' : 'Not found'}})
...
# @router.get("/{identification_card}", response_model = role_schema)
...
#                 #encontrar la primera coincidencia, esto es posible porque el id es un 
#                 #identificador unico
#                 r=db.query(role_model).where(role_model.identification_card == id_card).first()
#                 #r=db.select(role_model).where(role_model.identification_card == id_card)
#                 return r
#         #¡fin try!
#     except Exception as e:#instrucción que nos ayuda a atrapar la excepción que ocurre cuando alguna instrucción dentro de try falla
#             #se debe controlar siempre que nos conectamos a una base de datos con un try - except
